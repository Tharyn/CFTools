...
public class Bifröst : EditorWindow {
    ...
    public static void Launch() {
        window = GetWindow(typeof(Bifröst));
    ...
    void BakeLightmaps(bool selected) {
    ...
    public void OnGUI() {
        ...
        */
        if (GUI.Button(new Rect(10, 50, 80, 20), "Bake All")) {
            if (EditorUtility.DisplayDialog("Bake All", "ALL?", "Yes", "Cancle")) 
                BakeLightmaps(false);
            
        }

        if (GUI.Button(new Rect(90, 50, 80, 20), "Bake Sel")) {
            if (EditorUtility.DisplayDialog("Bake Selected", "SELECTED?", "Yes", "Cancle"))
                BakeLightmaps(true);
