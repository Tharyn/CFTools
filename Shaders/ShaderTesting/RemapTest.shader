...
/////// Diffuse:
                ...
                float3 diffuse = (directDiffuse + indirectDiffuse) * float3((float2(0,1)*1.8+0.2),0.0);

				//  float3((float2(0,1)*1.8+0.2),0.0);
/// Final Color:
...
/////// Diffuse:
                float NdotL = max(0.0,dot( normalDirection, lightDirection ));
                float3 directDiffuse = max( 0.0, NdotL) * attenColor;
                float3 diffuse = directDiffuse * float3((float2(0,1)*1.8+0.2),0.0);
/// Final Color:
                float3 finalColor = diffuse;
