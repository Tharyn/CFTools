...
////// Specular:
                ...
                float4 _SpecMap_var = tex2D(_SpecMap,TRANSFORM_TEX(i.uv0, _SpecMap));
                float3 specularColor = ((_SpecMap_var.rgb+(pow(1.0-max(0,dot(normalDirection, viewDirection)),_IOR)*_IORAmt))*_RefAmt);
...
////// Emissive:
                float3 emissive = ((_MainTex_var.rgb*_LightColor.rgb)*_LightIntensity*max(((_LightDistance-distance(_LightPos.rgb,i.posWorld.rgb))/_LightDistance),0.0)*_AOMap_var.rgb);
/// Final Color:
