...
/////// Vectors:
                ...
                float2 node_6669 = (_ParalaxAmt*(_HeightMap_var.a - .75)*mul(tangentTransform, viewDirection).xy + i.uv0);
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float2 node_6669 = (_ParalaxAmt*(_HiMap_var.a -.75)*mul(tangentTransform, viewDirection).xy + i.uv0);

				
				float4 _MainTex_var = tex2D(_MainTex, TRANSFORM_TEX(node_6669.rg, _MainTex) );
				float4 _SpecMap_var = tex2D(_SpecMap, TRANSFORM_TEX(node_6669.rg, _SpecMap) );
				float3 _BumpMap_var = UnpackNormal(tex2D(_BumpMap,TRANSFORM_TEX(node_6669.rg, _BumpMap)));
				float4 _AoMap_var = tex2D(_AoMap, TRANSFORM_TEX(node_6669.rg, _AoMap) );
